        ...
        try {
          ...
          const transformEvalsToItems = async (evals: Eval[]): Promise<EvalItem[]> => {
            ...
            return evals.map((evl) => {
              ...
              const passCount = prompts.reduce(
                (sum, p) => sum + (p.metrics?.testPassCount ?? 0),
                0,
              );
              const failCount = prompts.reduce(
                (sum, p) => sum + (p.metrics?.testFailCount ?? 0),
                0,
              );
          ...
          logger.debug(
            `Ink list failed, falling back to table: ${error instanceof Error ? error.message : error}`,
          );
        }
      }
